...
Page({
    ...
    onLoad: function (options) {
        ...
        wx.getStorage({
            ...
            success: (res) => {
                ...
                this.setData({quantity: options.quantity, allPrice: options.unitPrice * options.quantity, unitPrice: options.unitPrice})
                this.queryScenicDetail(options.scenicId)
            },
            fail: res => {
                wx.showToast({
                    title: '请先进行登录',
    ...
    submit() {
        ...
        setTimeout(() => {
            ...
            http.post('scenicOrderAdd', data).then((r) => {
                wx.showToast({
                    title: '支付成功',
                    icon: 'success',
                    duration: 1000
